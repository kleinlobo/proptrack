...
import { deleteExpenseRecord } from '@/app/(dashboard)/expenses/actions';
...
export function DeleteExpenseModal({ open, recordId, onClose, onSuccess }: Props) {
  ...
  function handleDelete() {
    ...
    startTransition(async () => {
      const result = await deleteExpenseRecord(recordId);
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-sm bg-white dark:bg-neutral-800 rounded-md shadow-xl p-0">
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-neutral-100 dark:border-neutral-700 mt-4">
          ...
          <Button type="button" onClick={handleDelete} disabled={isPending} className="bg-[var(--color-error)] hover:bg-red-700 text-white">
