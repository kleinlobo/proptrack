...
import { toMonthStart, toSupabaseDate, last12MonthRange, type DateRange } from '@/lib/date-utils';
...
export function useMonthlyChartData(propertyIds: string[] | null) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<MonthlyBarData[]> => {
      ...
      // Filter by propertyIds if set
      const revData = propertyIds?.length
        ? await supabase
            .from('revenue_summary_v')
            .select('month, total_amount_base')
            .gte('month', toMonthStart(range.from))
            .in('property_id', propertyIds)
            .then(({ data }) => data ?? [])
        : revRes;

      const expData = propertyIds?.length
        ? await supabase
            .from('expense_summary_v')
            .select('month, total_amount_base')
            .gte('month', toMonthStart(range.from))
      ...
      for (const row of revData) {
        ...
        cur.revenue += row.total_amount_base ?? 0;
      ...
      for (const row of expData) {
        ...
        const key = row.month.slice(0, 7);
        const cur = monthMap.get(key) ?? { revenue: 0, expenses: 0 };
        cur.expenses += row.total_amount_base ?? 0;
        monthMap.set(key, cur);
      }
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        result.push({ month: label, revenue: entry.revenue, expenses: entry.expenses, profit: entry.revenue - entry.expenses });
      }
...
export function useExpenseCategoryBreakdown(range: DateRange, propertyIds: string[] | null) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<CategoryBreakdown[]> => {
      let query = supabase
        .from('expense_summary_v')
        .select('category_parent, total_amount_base')
        .gte('month', toMonthStart(range.from))
        .lte('month', toMonthStart(range.to))
      ...
      for (const row of data ?? []) {
        ...
        catMap.set(name, (catMap.get(name) ?? 0) + (row.total_amount_base ?? 0));
      }
